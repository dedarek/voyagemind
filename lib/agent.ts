...
const SYSTEM_PROMPT = `你是一位专业、热情的旅行规划师，名叫"小途"。你的目标是通过循循善诱的对话，充分了解用户的旅行需求，然后利用工具获取真实数据，为用户制定个性化的旅行计划。
...
function cleanDSML(content: string): string {
  ...
  // DeepSeek 有时把 tool_calls 以内部 DSML 标记当文本输出
...
const TOOLS: ToolDef[] = [
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          keyword: { type: 'string', description: '搜索关键词，如"三亚酒店"、"亲子酒店"' },
          city: { type: 'string', description: '城市名称，如"三亚"、"北京"' },
        },
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          city: { type: 'string', description: '城市名称或高德行政区划代码，如"三亚"、"460200"、"北京"' },
        },
        required: ['city'],
      },
    },
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: '搜索关键词，如"三亚亚特兰蒂斯 值得去吗"、"西安必吃餐厅 推荐"、"亚龙湾酒店 避坑"' },
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          origin: { type: 'string', description: '起点坐标或地址，如"116.48,39.91"或"三亚凤凰机场"' },
...
async function callDeepSeek(
  ...
  if (!DEEPSEEK_API_KEY) {
    throw new Error('缺少 DEEPSEEK_API_KEY，请在 .env.local 中配置大模型 API Key')
  }
  ...
  if (!choice) throw new Error('DeepSeek 返回空响应')
...
export async function* runAgent(
  ...
  for (let psTurn = 0; psTurn < 2; psTurn++) {
    ...
    try {
      decision = await callDeepSeek(preMessages, TOOLS)
    } catch (err: unknown) {
  ...
  for (let turn = 0; turn < 3; turn++) {
    ...
    try {
      response = await callDeepSeek(messages, TOOLS, turn === 0 ? 'auto' : undefined)
    } catch (err: unknown) {
